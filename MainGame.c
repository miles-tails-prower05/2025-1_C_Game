...
int GameOver()
{
    ...
    printf(" ____                                                 _____                              \n");
    printf("/\\  _`\\                                              /\\  __`\\                            \n");
    printf("\\ \\ \\L\\_\\     __       ___ ___       __              \\ \\ \\/\\ \\    __  __     __    _ __  \n");
    printf(" \\ \\ \\L_L   /'__`\\   /' __` __`\\   /'__`\\             \\ \\ \\ \\ \\  /\\ \\/\\ \\  /'__`\\ /\\`'__\\\n");
    printf("  \\ \\ \\/, \\/\\ \\L\\.\\_ /\\ \\/\\ \\/\\ \\ /\\  __/              \\ \\ \\_\\ \\ \\ \\ \\_/ |/\\  __/ \\ \\ \\/ \n");
    printf("   \\ \\____/\\ \\__/.\\_\\\\ \\_\\ \\_\\ \\_\\\\ \\____\\              \\ \\_____\\ \\ \\___/ \\ \\____\\ \\ \\_\\ \n");
    printf("    \\/___/  \\/__/\\/_/ \\/_/\\/_/\\/_/ \\/____/               \\/_____/  \\/__/   \\/____/  \\/_/ \n");
    printf("                                                                                         \n");
    printf("                                                                                         \n");
    int y = 13, x = 4;
    gotoxy(2, y);
...
int Clear()
{
    ...
    printf(" 종 료 ");
    gotoxy(4, 10);
    printf("커서 변경이 안된다면 한/영키를 눌러주세요.");
    while (1) {
        int n = KeyControl();
        switch (n) {
        case UP: {
...
int DrawMenu() {
    ...
    printf("   종 료    ");
    gotoxy(32, 20);
    printf("WASD를 사용하여 이동하고 스페이스바를 눌러 선택하세요!");
    gotoxy(37, 21);
    printf("이동이 안된다면 한/영 키를 눌러주세요");
...
int GameStart() {
    ...
    if (good && !exit_loop)
    {
        ...
        case 4:
            ...
            gotoxy(27, 17);
            printf("도망간다면 앞으로 계속 괴롭힘에 시달릴 것이다.");
            x = 4, y = 20;
            gotoxy(2, y);
            printf("> 선 빵 필 승! ");
            gotoxy(4, y + 2);
            ...
            while (!exit_loop) {
                ...
                case SUBMIT: {
                    if (y == 20)
                    {
                        ...
                        gotoxy(15, 6);
                        printf("진실의 포션이 있었다면 결정적인 단서를 공유할 수도 있었을 것이다...");
                        gotoxy(0, 10);
                        printf("  > 넘 어 가 기 (스페이스바)    ");
                        KeyControl();
                        cls;
...
int Information()
{
    cls;
    gotoxy(30, 0);
    printf("스페이스바를 누르면 다음 이야기가 나옵니다.");
    gotoxy(30, 5);
    printf("평소 해리포터 소설을 즐겨보던 당신");
    KeyControl();
    gotoxy(30, 7);
    printf("어느 때처럼 해리포토 즐기던 중,");
    KeyControl();
    gotoxy(30, 9);
    printf("소설의 마지막 페이지를 넘긴 순간 책이 일렁거리더니 눈앞의 풍경이 순식간에 뒤바뀐다.");
    KeyControl();
    gotoxy(30, 11);
    printf("정신을 차리고 보니 나는 호그와트의 학생이 되어있었다.");
    KeyControl();
    gotoxy(30, 13);
    printf("해리포터가 볼드모트를 무찌른 뒤의 세계는 평화로운 듯 보였다.");
    KeyControl();
    gotoxy(30, 15);
    printf("원래의 세상으로 돌아갈 수 있는 방법은 전무한 상태,");
    KeyControl();
    gotoxy(30, 17);
    printf("당신은 호그와트에서 마법을 수련해 집에 돌아가는 주문을 개발하기로 한다.");
    KeyControl();
    main();
}
